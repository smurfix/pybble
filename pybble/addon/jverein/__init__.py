...
class VereinForm(Form):
	name = TextField('Name', [validators.required(u"Kein Verein ohne Name!"), validators.length(min=3, max=250)])
	database = TextField('Database', [validators.required(u"Die Datenbank mit den Mitglieder-Infos hätte ich doch gerne."), validators.length(min=3, max=30), sel_ok])
...
""" % (self.name,self.database)
	...
	def num_reg_mitglieder(self):
		return db.filter_by(Mitglied, parent_id=self.id, aktiv=True).count()
...
class MitgliedForm(Form):
	...
	email = TextField('Email', [validators.required(u"Ohne Mailadresse geht es nicht."), validators.length(min=3, max=150), verein_mail])
